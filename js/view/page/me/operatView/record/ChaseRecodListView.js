...
export default class ChaseRecodListView extends React.Component {
    ...
    render() {
        ...
        return (<View style={G_Style.appContentView}>
                <TFlatList curPage={curPage} totalPage={totalPage} pageSize={pageSize} dataList={dataList}
                           loadMore={this.props.loadMore} renderRow={this._renderRow}/>
            </View>)
    }
                ...
                <View style={styles.row}>
                    ...
                    <View style={[styles.itemContentStyle, {flex: 4, alignItems: "center", justifyContent: "center"}]}>
                        <Text style={styles.textItemStyle}
                              numberOfLines={1}>{`完成度:${rowData.finished_issues}/${rowData.total_issues}`}</Text>
                        {
                            userID != rowData.user_id ? <Text style={[styles.textItemStyle, {fontSize: 12, color: "red"}]}>
                                ({rowData.username})</Text> : null
                        }
                    ...
                    <View style={[styles.itemContentStyle, {flex: 2}]}>
                        <Text style={[styles.textItemStyle, {
                            ...
                            color: statusColor,
                            paddingRight: 3
                        }]}>{appModel.getATraceStatus(rowData.status)}</Text>
                    </View>
